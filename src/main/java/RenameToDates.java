...
public class RenameToDates {
    ...
    private static void tryRename(File jpegFile, String newName) {
        while (true) {
            final File destination = new File(jpegFile.getParent() + File.separator + newName + ".jpg");
            if (destination.exists()) {
                System.out.println(destination.getAbsolutePath() + " already exists.");
                if (destination.getAbsolutePath().equals(jpegFile.getAbsolutePath())) {
